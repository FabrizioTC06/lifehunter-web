document.addEventListener("DOMContentLoaded", () => {
  ...
  const plantas = document.querySelectorAll(".planta-grid a"); // puedes renombrar 'bacteria-grid' a algo más general si deseas
  ...
  const applyFilters = () => {
    ...
    const filo = filoSelect.value;
    const habitat = habitatSelect.value;
    const ciclo = cicloSelect.value;
    const habito = habitoSelect.value;
    const reproduccion = reproduccionSelect.value;
    const uso = usoSelect.value;

    plantas.forEach(link => {
      const planta = link.querySelector(".planta"); // puedes cambiar la clase .bacteria a .planta si renombraste en el HTML
      const nombrePlanta = planta.dataset.nombre.toLowerCase();
      const dataFilo = planta.dataset.filo;
      const dataHabitat = planta.dataset.habitat;
      const dataCiclo = planta.dataset.ciclo;
      const dataHabito = planta.dataset.habito;
      const dataReproduccion = planta.dataset.reproduccion;
      const dataUso = planta.dataset.uso;

      const matchesNombre = busqueda === "" || nombrePlanta.includes(busqueda);
      const matchesFilo = filo === "" || dataFilo === filo;
      const matchesHabitat = habitat === "" || dataHabitat === habitat;
      const matchesCiclo = ciclo === "" || dataCiclo === ciclo;
      const matchesHabito = habito === "" || dataHabito === habito;
      const matchesReproduccion = reproduccion === "" || dataReproduccion === reproduccion;
      const matchesUso = uso === "" || dataUso.includes(uso);

      if (matchesNombre && matchesFilo && matchesHabitat && matchesCiclo && matchesHabito && matchesReproduccion && matchesUso) {
  ...
  filoSelect.addEventListener("change", applyFilters);
  habitatSelect.addEventListener("change", applyFilters);
  cicloSelect.addEventListener("change", applyFilters);
  habitoSelect.addEventListener("change", applyFilters);
  reproduccionSelect.addEventListener("change", applyFilters);
  usoSelect.addEventListener("change", applyFilters);
  ...
  const aplicarFiltrosDesdeURL = () => {
    ...
    if (f.filo) filoSelect.value = f.filo;
    if (f.habitat) habitatSelect.value = f.habitat;
    if (f.ciclo) cicloSelect.value = f.ciclo;
    if (f.habito) habitoSelect.value = f.habito;
    if (f.reproduccion) reproduccionSelect.value = f.reproduccion;
    if (f.uso) usoSelect.value = f.uso;
