document.addEventListener("DOMContentLoaded", () => {
  ...
  const tipocelularSelect = document.getElementById("tipocelular");
  const nutricionSelect = document.getElementById("nutricion");
  const locomocionSelect = document.getElementById("locomocion");
  ...
  const plantas = document.querySelectorAll(".planta-grid a");
  ...
  const applyFilters = () => {
    ...
    const tipocelular = tipocelularSelect.value;
    const nutricion = nutricionSelect.value;
    const locomocion = locomocionSelect.value;
    const reproduccion = reproduccionSelect.value;
    const habitat = habitatSelect.value;

    plantas.forEach(link => {
      const planta = link.querySelector(".planta");
      const nombrePlanta = planta.dataset.nombre.toLowerCase();
      const dataSubgrupo = planta.dataset.subgrupo;
      const dataTipoCelular = planta.dataset.tipocelular;
      const dataNutricion = planta.dataset.nutricion;
      const dataLocomocion = planta.dataset.locomocion;
      const dataReproduccion = planta.dataset.reproduccion;
      const dataHabitat = planta.dataset.habitat;

      const matchesNombre = busqueda === "" || nombrePlanta.includes(busqueda);
      const matchesSubgrupo = subgrupo === "" || dataSubgrupo === subgrupo;
      const matchesTipoCelular = tipocelular === "" || dataTipoCelular === tipocelular;
      const matchesNutricion = nutricion === "" || dataNutricion === nutricion;
      const matchesLocomocion = locomocion === "" || dataLocomocion === locomocion;
      const matchesReproduccion = reproduccion === "" || dataReproduccion === reproduccion;
      const matchesHabitat = habitat === "" || dataHabitat === habitat;
  ...
  botonReiniciar.addEventListener("click", () => {
    ...
    tipocelularSelect.value = "";
    nutricionSelect.value = "";
    locomocionSelect.value = "";
    reproduccionSelect.value = "";
    habitatSelect.value = "";
    applyFilters();
  });

  subgrupoSelect.addEventListener("change", applyFilters);
  tipocelularSelect.addEventListener("change", applyFilters);
  nutricionSelect.addEventListener("change", applyFilters);
  locomocionSelect.addEventListener("change", applyFilters);
  ...
  const getParametrosURL = () => {
    ...
    return {
      ...
      tipocelular: params.get('tipocelular'),
      nutricion: params.get('nutricion'),
      locomocion: params.get('locomocion'),
  ...
  const aplicarFiltrosDesdeURL = () => {
    ...
    if (f.subgrupo) subgrupoSelect.value = f.subgrupo;
    if (f.tipocelular) tipocelularSelect.value = f.tipocelular;
    if (f.nutricion) nutricionSelect.value = f.nutricion;
    if (f.locomocion) locomocionSelect.value = f.locomocion;
    if (f.reproduccion) reproduccionSelect.value = f.reproduccion;
    if (f.habitat) habitatSelect.value = f.habitat;
